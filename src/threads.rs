...
pub struct TaskWorker {
    ...
    quality: i32,
    dir_name: PathBuf,
    main_worker: Worker<Option<DirEntry>>,
    task_amount: usize,
...
impl TaskWorker {
    ...
    pub fn send_to_threads(mut self) -> Option<Vec<thread::JoinHandle<()>>> {
        let mut handles = Vec::with_capacity(usize::from(self.device_num));
        let to_steal_from = Arc::new(Mutex::new(self.thread_stealers));
        for _ in 0..self.device_num {
